...
export function TerminalLine({ index, year, title, summary, url, delay, startAnimation = true }: TerminalLineProps) {
  ...
  useEffect(() => {
    ...
    const fetchNoteData = async () => {
      try {
        const response = await fetch(`https://mx.innei.in/api/v2/notes/nid/${noteId}`)
  ...
  const size = noteData ? formatSize(noteData.size) : `${(1024 + index * 256).toString().padStart(4, " ")}`
  ...
  const content = (
    <>
      {/* 桌面端：权限和元信息行 */}
      <div className="hidden sm:flex font-mono text-sm text-muted-foreground/70 items-center gap-x-3 leading-none">
      ...
      {/* 移动端：简化的年份显示 */}
      <div className="sm:hidden font-mono text-sm text-muted-foreground/70 flex items-center gap-2 leading-none">
        <span className={`text-primary font-medium ${isHovered ? "animate-pulse" : ""}`}>{year}.md</span>
      ...
      {/* 标题和摘要行 */}
      <div className="font-mono pl-0 sm:pl-6 mt-1 sm:mt-0.5">
